...
describe('*** TEST MODEL1  APIs:', function () {
var token , id, email, signid
  ...
  describe('GET '+entitytest1, function () {
    it('should return 200 OK , {error: false, message: "OK", result: [{TestModel1}]}', function (done) {
      server
        .get(entitytest1)
  ...
  describe('POST '+entitytest1, function () {
    it('should return 200 OK, {error: false, message: "OK", result: {TestModel1}}', function (done) {
      server
        .post(entitytest1)
        .set('x-access-token', token)
        .send({ call: "disconnected", callCode: 123456 })
        .expect('Content-Type', /json/)
        .expect(200)
        .end( function(err, res) {
          res.body.error.should.equal(false);
  ...
  describe('PUT '+entitytest1, function () {
    it('should return 200 OK, {error: false, message: "OK", result: []}', function (done) {
      server
        .put(entitytest1)
        .set('x-access-token', token)
        .send({id : id, call: "disconnected", callCode: 123456})
        .expect('Content-Type', /json/)
        .expect(200)
        .end( function(err, res) {
          res.body.error.should.equal(false);
