...
var entitylisto = "/api/listo";
var entityhook = "/hooks/listo";
...
describe('*** LISTO APIs:', function () {
var token , id, email, signid, entityid
  ...
  describe('POST  listo '+entitylisto, function () {
    it('should return 200 OK, {error: false, message: "OK", result: {listo}}', function (done) {
      server
        .post(entitylisto)
        .set('x-access-token', token)
        .send({rfc : 'test', customer_token : '1234567', entity_id : entityid})
        .expect('Content-Type', /json/)
        .expect(200)
        .end( function(err, res) {
          res.body.error.should.equal(false);
 ...
 /* describe('POST  hook '+ entityhook, function () {
    this.timeout(10000)
    it('should return 200 OK, {error: false, message: "OK", result: {hooklisto}}', function (done) {
      this.timeout(10000)
      server
        .post(entityhook)
        .set('x-access-token', token)
